...
def get_tr_id():
    '''
    Get locally unique transaction id
    tr_id is currently incrementing global
    counter
    '''
...
def parse_M3RDF(results):
    '''
    Helper function to parse M3 RDF encoding
    '''
    results_list = []
    parser = make_parser()
...
def parse_URI_list(results):
    '''
    Helper function to parse URI lists
    (returned by some wql queries)
    '''
    results_list = []
    parser = make_parser()
...
def parse_sparql(stringa):
    '''
    Helper function to parse SPARQL query results
    '''
    if stringa == "":
        return []
                    ...
                    for bind_n in ris.getElementsByTagName('binding'):
                        ...
                        if tipo == "literal":
                            ...
                            if bind_n.getElementsByTagName('bnode')[0].firstChild==None:
                                ...
                                elemento = [nome,
                                            tipo,
                                            bind_n.getElementsByTagName('bnode')[0].firstChild.data]
                        elif tipo == "unbound":
                            elemento = [nome, tipo, None]
...
class KPError(M3Exception):
    '''
    Exception for KP related error
    '''
...
class bNode(Node):
    '''
    Class representing blank nodes (bNodes)
    '''
    def __init__(self, value):
        self.value = value
        self.nodetype = "bnode"
...
class Triple(tuple):
    ...
    def encode_to_m3(self):
        '''
        Method to provide a serialization in M3 format
        '''
        s, p, o = self
        for i in self:
            if i == None:
...
class KP:
    '''
    Class to represent a Smart-M3 knowledge processor
    '''
...
class Subscribe(Transaction):
    '''
    Class handling subscribe transactions
    '''
...
class Connector:
    '''
    Abstract base class for connectors that handle
    the network connections to SIB
    '''
    def connect(self):
...
class TCPConnector(Connector):
    '''
    Class handling TCP connections to SIB
    '''
    ...
    def send(self, msg):
        length = len(msg)
        sent = self.s.send(msg.encode('UTF-8'))
	#print msg
        while sent < length:
    ...
    def receive(self):
        ...
        while True:
            ...
            if msg_end_index == -1: # Received partial msg
                ...
                #print msg
                msg_unicode = u""
                msg_unicode = msg.encode('unicode_escape')
                msg_ascii = msg_unicode.decode('ascii','xmlcharrefreplace')
